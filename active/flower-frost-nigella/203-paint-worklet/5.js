    ...
    for (let x = 0; x < width; x += gridSize) {
      ...
      for (let y = 0; y < height; y += gridSize) {
        ...
        for (let i = 0; i < count; i++) {
          const numPoints = Math.floor(randomItem.nextBetween(6, 8));
          const points = [];
          const angleStep = Math.PI * 2 / numPoints;
          const center = new Vector2D(x + randomItem.nextBetween(0, gridSize), y + randomItem.nextBetween(0, gridSize));
          let radius = baseSize;
          ...
          for (let i2 = 1; i2 <= numPoints; i2++) {
            ...
            points.push(Vector2D.lerp(point, center, randomItem.nextBetween(0, 0.625)));
          }
          const fill = colors[Math.floor(randomItem.nextBetween(0, colors.length))];
          ctx.fillStyle = fill;
          ctx.beginPath();
