    ...
    for (let x = 0; x < width; x += gridSize) {
      ...
      for (let y = 0; y < height; y += gridSize) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const color = colors[Math.floor(randomItem.nextBetween(0, colors.length))];
          
          drawBlob(ctx, {
            random: randomItem,
            x: x + randomItem.nextBetween(0, gridSize),
            y: y + randomItem.nextBetween(0, gridSize),
            size: radius,
            color,
          });
        }
