if (document.readyState === "complete" || document.readyState !== "loading" && !document.documentElement.doScroll) {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      const codecsStringDisplay=document.getElementById('CodecsString');
      ...
      uploadMedia.addEventListener('change', async(evt)=> {
        ...
        await ffmpeg.run('-i', fileName, '-c:v copy', '-bsf:v trace_headers', `-f dump.txt`);
        const dump = ffmpeg.FS('readFile', `dumpt.txt`); // Uint8Array 
        const textDecoder = new TextDecoder();
        appendDataLog(textDecoder.decode(dump));
