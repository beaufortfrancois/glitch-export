...
turnOnBrkSignalButton.onclick = () => setSignals({ brk: true });
turnOnDtrSignalButton.onclick = () => setSignals({ dtr: true });
turnOnRtsSignalButton.onclick = () => setSignals({ rts: true });
turnOffBrkSignalButton.onclick = () => setSignals({ brk: false });
turnOffDtrSignalButton.onclick = () => setSignals({ dtr: false });
turnOffRtsSignalButton.onclick = () => setSignals({ rts: false });
...
getSignalsButton.onclick = async event => {
  ...
  console.log(`Data Carrier Detect: ${signals.dcd}`);
  console.log(`Clear To Send: ${signals.cts}`);
  console.log(`Ring Indicator: ${signals.ri}`);
  console.log(`Data Set Ready: ${signals.dsr}`);
};
