...
self.addEventListener('install', (event) => {
  ...
  // cache key, but the actual URL we end up requesting might include a
...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.mode === 'navigate' ||
    ...
    event.respondWith(
        fetch(event.request).catch((error) => {
          ...
          // If fetch() returns a valid HTTP response with an response code
          // in the 4xx or 5xx range, the catch() will NOT be called. If you
          // need custom handling for 4xx or 5xx errors.
  ...
  // the request. If there are any other fetch handlers registered, they will
  // get a chance to call event.respondWith(). If no fetch handlers call
  // event.respondWith(), the request will be handled by the browser as if
